...
### 15호 — 확대 뷰 없이 + 고주파만 (03:25 KST)
노멀 오차 7.6°, 가장 깨끗하고 매끈한 조각상 느낌. 대신 확대 뷰가 없어 눈·책 장식은 없음.
고주파만 쓰는 고침이 확대 뷰와 상관없이 전체적으로 이득이라는 확인.

![owl15](owl15.png)

### 16호 — 경계 띠 (03:50 KST)
14호와 거의 동일(9.4°). 가슴 가로 이음매 그대로: 다음 묶음을 "덧얹기"만 했지 주인 묶음은
자기 프레임 끝까지 무게 1 → 여전히 딱 끊김. 고침: 주인은 서서히 빠지고 다음이 서서히
들어오는 **교차 페이드**. → 18호(핑퐁 3라운드 뷰 + 교차 페이드), 17호(3라운드 뷰 + 옛 띠).

### 근본 원인 (04:15 KST, 사용자 지적: 귀 으스러짐, 발이 책에 붙음, 날개 잘림)
위: 시작점인 실루엣 헐(6뷰 교집합), 아래: 15호 최종. **최종 메쉬가 헐의 각진 덩어리를 그대로
갖고 있음** — 평평한 얼굴판, V자 눈썹, 평평한 정수리, 헐의 날개깃 같은 귀.

![hull vs 15](hull_vs_owl15.png)

학습 중 렌더(위) vs 정답 노멀(아래): 정답엔 얼굴·부리·뾰족한 귀가 있는데 메쉬는 평면.

![snap heads](snap_heads_owl15.png)

- 실루엣은 **바깥 윤곽**만 알려줌. 얼굴 앞면, 오목한 얼굴판, 발가락과 책 사이 틈, 귀깃의 옆모습은
  실루엣으로 못 봄 → 헐 상태 그대로 = 평평한 판.
- 그걸 조각해야 할 게 노멀인데, 최적화가 **꼭짓점마다 따로** 움직임(Adam). 노멀이 "얼굴 전체를
  2cm 앞으로"를 원해도 각 꼭짓점은 자기 삼각형만 살짝 기울임 → 헐 위에 **결만 그려진** 상태.
- 거기에 14호의 `--hp-world 0.01`이 "1cm보다 큰 형태는 헐(형태 단계)을 믿어라"로 못을 박음.
- 날개 잘림: 확대 뷰 경계의 이음매(부차적).

고침(`--sobolev`): 형태 단계의 기울기를 표면 위에서 부드럽게 펴서(I+λL)⁻¹ 영역 전체가 한 덩어리로
움직이게 함(Nicolet 2021 "Large Steps"). 헐에서 새로 시작, 에지 12→6mm, 600스텝,
디테일 단계 `--hp-world 0.03`. → 20호.
