...
### 원칙 4 — 대칭은 공짜 데이터다 (단, 선택 사항)

**갱신:** 대칭은 이제 필수가 아니라 켜고 끄는 스위치입니다. GT 테스트 대상으로
고른 Lucy(Stanford 천사상)가 비대칭이라 — 한쪽 팔만 들고 있고, 옷주름이 좌우
다릅니다 — 하네스는 대칭 없이도 돌아야 합니다.

대칭 가능한 대상이면 형상에 하드 제약을 겁니다:
...
**비대칭 대상일 때** (`--symmetry off`): 이 2배 중복이 없어집니다. 그만큼을
나머지로 메꿔야 해요 —
- hull 외피 제약의 가중을 올립니다 (실루엣은 대칭과 무관하게 정확하므로)
- normal 손실 가중을 올리고 깊이 가중을 더 낮춥니다
- 평활/eikonal 정규화를 소폭 강화
- 그래도 부족하면 3/4 보조뷰가 대칭 대신 중복을 공급합니다 (§3 선택지 b)

즉 대칭은 **있으면 크게 이득, 없으면 보조뷰로 대체 가능한 항목**이지 전제가
아닙니다. 파이프라인 구조는 그대로입니다.
